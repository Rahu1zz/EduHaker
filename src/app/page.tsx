"use client";

import { QuizItem } from "@/components/QuizItems";
import { nanoid } from "nanoid";
import { useState } from "react";
import { Quiz } from "../types/quiz";

/**
 *
 * 1) CopilotKit Integration
 *
 **/

import {
  CopilotKit,
  useCopilotAction,
  useCopilotReadable,
} from "@copilotkit/react-core";
import { CopilotPopup } from "@copilotkit/react-ui";
import "@copilotkit/react-ui/styles.css";
import Link from "next/link";

export default function Home() {
  return (
    <div className="border rounded-md max-w-2xl mx-auto p-4">
      <h1 className="text-2xl font-bold ">Hello Quizers 🪁</h1>
      <h2 className="text-base font-base mb-4">Todo Quizes</h2>

      {/**
       *
       * 2) Wrap the TodoList component with CopilotKit
       *
       **/}

      <CopilotKit
        publicApiKey={process.env.NEXT_PUBLIC_COPILOT_CLOUD_API_KEY}
        // Alternatively, you can use runtimeUrl to host your own CopilotKit Runtime
        // runtimeUrl="/api/copilotkit"
      >
        <TodoList />

        {/**
         *
         * 3) Add the CopilotPopup component to get the chat
         *
         */}

        <CopilotPopup
          instructions={
            "Help the user manage a quiz list. If the user provides a high level goal, " +
            "break it down into a few specific small quizes and add them to the list"
          }
          defaultOpen={true}
          labels={{
            title: "Quiz List Copilot",
            initial: "Hi you! 👋 I can help you manage your Quizes list.",
          }}
          clickOutsideToClose={false}
        />
      </CopilotKit>
    </div>
  );
}

const TodoList: React.FC = () => {
  const [quizs, setQuizs] = useState<Quiz[]>([]);
  const [input, setInput] = useState("");

  /**
   *
   * 4) make the users todo list available with useCopilotReadable
   *
   **/
  useCopilotReadable({
    description: "The user's todo list.",
    value: quizs,
  });

  /**
   *
   * 5) Add the useCopilotAction to enable the copilot to interact with the todo list
   *
   **/

  useCopilotAction({
    name: "updateTodoList",
    description: "Update the users todo list",
    parameters: [
      {
        name: "items",
        type: "object[]",
        description: "The new and updated todo list items.",
        attributes: [
          {
            name: "id",
            type: "string",
            description:
              "The id of the todo item. When creating a new todo item, just make up a new id.",
          },
          {
            name: "text",
            type: "string",
            description: "The text of the todo item.",
          },
          {
            name: "isCompleted",
            type: "boolean",
            description: "The completion status of the todo item.",
          },
          {
            name: "assignedTo",
            type: "string",
            description:
              "The person assigned to the todo item. If you don't know, assign it to 'YOU'.",
            required: true,
          },
        ],
      },
    ],
    handler: ({ items }) => {
      console.log(items);
      const newQuizes = [...quizs];
      for (const item of items) {
        const existingItemIndex = newQuizes.findIndex(
          (todo) => todo.id === item.id
        );
        if (existingItemIndex !== -1) {
          newQuizes[existingItemIndex] = item;
        } else {
          newQuizes.push(item);
        }
      }
      setQuizs(newQuizes);
    },
    render: "Updating the Quiz list...",
  });

  /**
   *
   * 5) Add another useCopilotAction to enable the copilot to delete a todo item
   *
   **/
  useCopilotAction({
    name: "deleteQuiz",
    description: "Delete a Quiz item",
    parameters: [
      {
        name: "id",
        type: "string",
        description: "The id of the Quiz item to delete.",
      },
    ],
    handler: ({ id }) => {
      setQuizs(quizs.filter((quiz) => quiz.id !== id));
    },
    render: "Deleting a Quiz item...",
  });

  const addQuiz = () => {
    if (input.trim() !== "") {
      // Check if input is not just whitespace
      const newQuiz: Quiz = {
        id: nanoid(),
        text: input.trim(), // Trim whitespace
        isCompleted: false,
      };
      setQuizs([...quizs, newQuiz]);
      setInput(""); // Reset input field
    }
  };

  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === "Enter") {
      addQuiz();
    }
  };

  const toggleComplete = (id: string) => {
    setQuizs(
      quizs.map((quiz) =>
        quiz.id === id ? { ...quiz, isCompleted: !quiz.isCompleted } : quiz
      )
    );
  };

  const deleteQuiz = (id: string) => {
    setQuizs(quizs.filter((quiz) => quiz.id !== id));
  };

  const assignPerson = (id: string, person: string | null) => {
    setQuizs(
      quizs.map((quiz) =>
        quiz.id === id
          ? { ...quiz, assignedTo: person ? person : undefined }
          : quiz
      )
    );
  };

  return (
    <div>
      <div className="flex mb-4">
        <input
          className="border rounded-md p-2 flex-1 mr-2"
          value={input}
          onChange={(e) => setInput(e.target.value)}
          onKeyDown={handleKeyPress} // Add this to handle the Enter key press
        />
        <button
          className="bg-blue-500 rounded-md p-2 text-white"
          onClick={addQuiz}
        >
          Add Todo
        </button>
      </div>
      {quizs.length > 0 && (
        <div className="border rounded-lg">
          {quizs.map((quiz, index) => (
            <QuizItem
              key={quiz.id}
              quiz={quiz}
              toggleComplete={toggleComplete}
              deleteQuiz={deleteTodo}
              assignPerson={assignPerson}
              hasBorder={index !== quizs.length - 1}
              onClick={() => {
                <Link href={`/Quiz/${quiz.id}`}></Link>;
              }}
            />
          ))}
        </div>
      )}
    </div>
  );
};
